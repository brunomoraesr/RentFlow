...
function renderTenantsTable() {
  ...
  const desc  = document.getElementById('tenantsDesc');
  if (!tbody) return;

  const search     = (document.getElementById('filterTenantSearch')?.value || '').toLowerCase().trim();
  const dateFilter = document.getElementById('filterTenantDate')?.value || '';

  const filtered = allTenants.filter(t => {
    if (search && !t.name.toLowerCase().includes(search) && !t.phone.includes(search)) return false;
    if (dateFilter && t.contract_start !== dateFilter) return false;
    return true;
  });

  if (!allTenants.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="tenants-empty">Nenhum inquilino cadastrado ainda.</td></tr>';
    if (desc) desc.textContent = 'Nenhum inquilino cadastrado';
    return;
  }

  if (desc) desc.textContent = `${filtered.length} de ${allTenants.length} inquilino${allTenants.length !== 1 ? 's' : ''}`;

  if (!filtered.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="tenants-empty">Nenhum resultado para os filtros aplicados.</td></tr>';
    return;
  }

  tbody.innerHTML = filtered.map(t => {
    const propName = t.properties?.name ?? '—';
    const regDate  = new Date(t.created_at).toLocaleDateString('pt-BR');
    const conStart = t.contract_start
      ? new Date(t.contract_start + 'T00:00:00').toLocaleDateString('pt-BR')
      : '—';
    return `<tr>
      <td>${escHtml(t.name)}</td>
      <td>${escHtml(t.cpf)}</td>
      <td>${escHtml(propName)}</td>
      <td>${conStart}</td>
      <td>${regDate}</td>
      <td><button class="btn-tenant-data" onclick="openTenantDetail('${t.id}')">Ver dados</button></td>
    </tr>`;
  }).join('');
}

function clearTenantFilters() {
  const s = document.getElementById('filterTenantSearch');
  const d = document.getElementById('filterTenantDate');
  if (s) s.value = '';
  if (d) d.value = '';
  renderTenantsTable();
}
...
function openTenantDetail(id) {
  ...
  document.getElementById('tenantDetailBody').innerHTML = `
    <div class="td-grid">
      <div class="td-field full">
        <span class="td-label">Nome completo</span>
        <span class="td-value">${escHtml(t.name)}</span>
      </div>
      <div class="td-field">
        <span class="td-label">CPF</span>
        <span class="td-value">${escHtml(t.cpf)}</span>
      </div>
      <div class="td-field">
        <span class="td-label">Telefone</span>
        <span class="td-value">${escHtml(t.phone)}</span>
      </div>
      <div class="td-field">
        <span class="td-label">E-mail</span>
        <span class="td-value">${t.email ? escHtml(t.email) : '—'}</span>
      </div>
      <div class="td-field">
        <span class="td-label">Imóvel</span>
        <span class="td-value">${escHtml(propName)}</span>
      </div>
      <div class="td-field">
        <span class="td-label">Início do contrato</span>
        <span class="td-value">${conStart}</span>
      </div>
      <div class="td-field">
        <span class="td-label">Cadastrado em</span>
        <span class="td-value">${regDate}</span>
      </div>
    </div>`;
  document.getElementById('modalTenantDetail').classList.add('open');
}

function closeTenantModal(e) {
  if (e && e.target !== document.getElementById('modalTenantDetail')) return;
  document.getElementById('modalTenantDetail').classList.remove('open');
}
